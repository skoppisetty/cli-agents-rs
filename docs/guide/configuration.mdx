...
- **Antigravity** — unsupported in headless mode; setting either field returns an error
...
Antigravity does not expose a per-run MCP configuration path. It loads global
servers from `~/.gemini/config/mcp_config.json` and workspace servers from
`.agents/mcp_config.json`. When Antigravity is selected, a non-empty
`RunOptions.mcp_servers` returns an error without changing the working directory
or either configuration file.
...
| `max_consecutive_tool_failures` | disabled | Number of consecutive tool failures before aborting; `0` disables the limit |
| `max_output_bytes` | 10 MB | Per-line stdout retention cap — an oversized line is dropped (surfaced as a warning event) and the run continues; cumulative throughput is unbounded by design |
...
```rust
use cli_agents::{RunOptions, ProviderOptions, AntigravityOptions, CliName};

let opts = RunOptions {
    cli: Some(CliName::Antigravity),
...
See [`AntigravityOptions`](/api/types#antigravityoptions) for all fields.
