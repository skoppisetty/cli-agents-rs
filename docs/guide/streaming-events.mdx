...
Every agent run emits a stream of [`StreamEvent`](/api/events) values through the callback you pass to `run()`. All three providers (Claude, Codex, Antigravity) are normalized into the same event types.
...
| Flag | Description |
|------|-------------|
| `--cli <name>` | Which CLI to use (`claude`, `codex`, `antigravity`) — auto-discovers if omitted |
