...
Serializes to lowercase strings: `"claude"`, `"codex"`, `"antigravity"`. Implements `Display` with the same output. Antigravity is discovered via its installed `agy` executable.
...
| `mcp_servers` | `None` | MCP servers to connect (Claude/Codex); Antigravity uses preconfigured workspace/global MCP files and rejects a non-empty value |
...
| `max_consecutive_tool_failures` | disabled | Max consecutive tool failures before aborting; `0` disables the limit |
...
| `max_output_bytes` | 10 MB | Per-line stdout retention cap; oversized lines are dropped, the run continues |
...
| `agent` | Custom agent name, as listed by `agy agents` |
| `sandbox` | Enable sandbox mode |
| `print_timeout` | Headless timeout (for example, `"10m"`) |
| `extra_args` | Additional CLI arguments passed directly to the `agy` binary |
